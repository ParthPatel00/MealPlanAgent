...
def generate_charts():
    ...
    tools = ["recipe_search", "allergy_checker", "nutrition", "grocery_list", "budget_estimator", "ics_generator"]
    tool_times_pct = [35, 25, 5, 5, 5, 5]  # approximate % of pipeline time
    colors_t = ["#3498db", "#e74c3c", "#2ecc71", "#9b59b6", "#f39c12", "#1abc9c"]
    left = 0
...
def build_report():
    ...
    pdf.code_block(
        ...
        "    v\n"
        "[EXECUTOR] -- 6 deterministic tools:\n"
    ...
    pdf.code_block(
        ...
        '            "Build grocery list", "Estimate budget", "Generate calendar"]\n'
        '}'
    )
    ...
    pdf.code_block(
        ...
        "\n"
        "Tool 10: ics_generator(cooking_blocks)\n"
        "  -> Generated .ics with 3 calendar events (Mon/Wed/Fri at 6 PM)"
    )
    ...
    pdf.body_text(
        "The Streamlit web UI provides 7 tabs covering the full user experience: "
        "meal plan display with PDF download, grocery list with budget visualization, "
        "nutrition charts, calendar export, full agent trace (showing LLM thinking), "
        "memory/preferences management, and model comparison dashboard."
    )
    pdf.body_text(
    ...
    pdf.code_block(
        ...
        "Tools:           6 (recipe_search, allergy_checker, nutrition,\n"
        "                    grocery_list, budget_estimator, ics_generator)\n"
        "Models:          3 (Llama 70B cloud, Llama 3B local, Granite 2B local)\n"
        "Memory:          SQLite with write/summarize/retrieve pattern\n"
        "UI:              Streamlit with 7 tabs + PDF/ICS export"
    )
